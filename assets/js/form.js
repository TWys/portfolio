...
function nameValidation() {
    ...
    if (name.length >= 1) {
        if (name.length <= 3) {
            ...
            document.getElementById('send_form_status').innerHTML = "Proszę wpisać conajmniej 4 znaki w polu 'Imię i Nazwisko'";
...
function mailValidation() {
    ...
    if (email.length >= 1) {
        ...
        if (regexp.test(email) === false) {
            ...
            document.getElementById('send_form_status').innerHTML = "Nieprawidłowy adres email. Popraw proszę";
            document.getElementById('send_form_status').style.color = "red";
        }
        else {
